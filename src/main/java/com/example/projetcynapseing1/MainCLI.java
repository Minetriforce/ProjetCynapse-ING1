 ...
 * 
 * <p>
 * Color codes are used for improved CLI visual output.
 * </p>
 * 
 * @author Jonathan
 */
public class MainCLI {
    /**
     * Controller for managing maze operations such as creation, loading, solving,
     * and saving
     */
    ...
    /**
     * Prompts the user with the given message to enter an integer that is zero or
     * positive.
     * Continuously reads input from the provided Scanner until a valid integer >= 0
     * is entered.
     * If the input is invalid or negative, an error message is displayed and the
    ...
    /**
     * Continuously prompts the user to input an integer within the valid range [0,
     * rows * columns - 1].
     * Reads from the provided Scanner until a valid integer is entered.
     * If the input is not an integer or out of the valid range, displays an error
     * message and prompts again.
     *
     * @param sc      The Scanner object used to read user input.
     * @param rows    The number of rows in the maze (used to determine the upper
     *                bound).
     * @param columns The number of columns in the maze (used to determine the upper
     *                bound).
        ...
        while (true) {
            if (sc.hasNextInt()) {
                ...
                if (value >= 0 && value < rows * columns) {
                    ...
                    System.out.println(
                            RED + "Please enter an integer between 0 and " + ((rows * columns) - 1) + ": " + RESET);
                }
            } else {
    ...
    /**
     * Allows the user to add or remove a wall between two adjacent cells in the
     * maze.
    ...
    public static void main(String args[]) {
        ...
        // Variables to store the number of cell of the solution path and the number of
        ...
        while (true) {
            ...
            if (menuChoice.equals("1") || menuChoice.equals("generate") ||
                ...
                System.out.println(
                        RED + "Invalid choice. Please enter the first word or the corresponding number" + RESET);
            }
        }
        ...
        if (menuChoice.equals("1") || menuChoice.equals("generate")) {
            ...
            while (true) {
                ...
                if (generationChoice.equals("1") || generationChoice.equals("prim")
                    ...
                    System.out
                            .println(RED + "Invalid choice. Please enter the name or the corresponding number" + RESET);
                }
            }
        }
        ...
        System.out.println(ITALIC + "Do you want to edit walls in the maze? " + RESET + BOLD + "[" + GREEN + "Y" + RESET
                + "/" + RED + "N" + RESET + "]");
        ...
        if (editWallsChoice.equals("y")) {
            while (true) {
                ...
                System.out.println(ITALIC + "Edit another wall? " + RESET + BOLD + "[" + GREEN + "Y" + RESET + "/" + RED
                        + "N" + RESET + "]");
        ...
        System.out.println(ITALIC + "Solve the maze? " + RESET + BOLD + "[" + GREEN + BOLD + "Y" + RESET + "/" + RED
                + BOLD + "N" + RESET + "]" + RESET);
        ...
        if (solveChoice.equals("y")) {
            ...
            switch (solvingChoice) {
                ...
                case "astar":
                    ...
                    mazeController.findSolution(MethodName.SolveMethodName.ASTAR, maze.getVertexByID(startId),
                ...
                case "bfs":
                    ...
                    startTime = System.currentTimeMillis();
                    mazeController.findSolution(MethodName.SolveMethodName.BFS, maze.getVertexByID(startId),
                            maze.getVertexByID(endId));
                    endTime = System.currentTimeMillis();
                    break;
                ...
                case "dfs":
                    ...
                    startTime = System.currentTimeMillis();
                    mazeController.findSolution(MethodName.SolveMethodName.DFS, maze.getVertexByID(startId),
                            maze.getVertexByID(endId));
                    endTime = System.currentTimeMillis();
                    break;
                case "4":
                case "righthand":
                    ...
                    startTime = System.currentTimeMillis();
                    mazeController.findSolution(MethodName.SolveMethodName.RIGHTHAND, maze.getVertexByID(startId),
                            maze.getVertexByID(endId));
                    endTime = System.currentTimeMillis();
                    break;
                case "5":
            ...
            System.out.println(mazeController.getSolver() + ":  Path Length: " + pathCount + " | " + "Visited: "
                    + visitedCount + "| Time: " + timeMs + " ms");

        }

        System.out.println(ITALIC + "Save the maze? " + RESET + BOLD + "[" + GREEN + BOLD + "Y" + RESET + "/" + RED
                + BOLD + "N" + RESET + "]" + RESET);
